...
const { removeSizeInOption } = productApi;

const ModalDeleteOption = ({ showModal, setShowModal, size }) => {
  const [isloading, setIsloading] = useState(false);

  const handleConfirm = () => {
    setIsloading(true);
    removeSizeInOption(size.id)
      .then((resp) => {
        if (resp.status === "OK") {
          notify(1, "Xóa size thành công!");
        } else {
      ...
      .catch((error) => {
        console.log(error);
        notify(0, "Xóa size không thành công!");
      })
      .finally(() => {
        setIsloading(false);
        setShowModal(false);
          ...
          <div className="relative w-auto my-2 mx-auto max-w-4xl">
            <div className="h-[220px] w-[300px] border-0 rounded-lg shadow-lg relative flex flex-col bg-white outline-none focus:outline-none">
              ...
              <div className="mx-4 mt-3 font-medium text-center">
                Bạn có muốn xóa size này không?
              </div>
