...
const { updateQuantitySizeOption } = productApi;

const ModalUpdateOptionQuantity = ({ showModal, setShowModal, size }) => {
  const [isloading, setIsloading] = useState(false);

  const handleConfirm = () => {
    setIsloading(true);
    updateQuantitySizeOption(size.id, { quantity: size.quantity })
      .then((resp) => {
        if (resp.status === "OK") {
          notify(1, "Cập nhật số lượng sản phẩm thành công!");
        } else {
      ...
      .catch((error) => {
        console.log(error);
        notify(0, "Cập nhật số lượng sản phẩm không thành công!");
      })
      .finally(() => {
        setIsloading(false);
        setShowModal(false);
          ...
          <div className="relative w-auto my-2 mx-auto max-w-4xl">
            <div className="h-[220px] w-[300px] border-0 rounded-lg shadow-lg relative flex flex-col bg-white outline-none focus:outline-none">
              ...
              <div className="mx-4 mt-3 font-medium text-center">
                Bạn có muốn cập nhật số lượng của size này không?
              </div>
...
export default ModalUpdateOptionQuantity;
