...
const { disableProduct } = productApi;

const ModalDisabledProduct = ({ setShowModal, showModal, id }) => {
    const handleConfirm = () => {
        disableProduct(id)
      .then((resp) => {
        if (resp.status === "OK") {
          setShowModal(false);
          notify(1, "Vô hiệu sản phẩm thành công");
          console.log(resp);
        } else {
          setShowModal(false);
          notify(0, "Vô hiệu sản phẩm thất bại");
        }
      })
      .catch((err) => {
        setShowModal(false);
            ...
            <div className="relative w-auto my-2 mx-auto max-w-4xl">
              <div className="h-[220px] w-[300px] border-0 rounded-lg shadow-lg relative flex flex-col bg-white outline-none focus:outline-none">
                ...
                <div className="mx-4 mt-3 font-medium text-center">
                  Bạn có muốn vô hiệu sản phẩm này không?
                </div>
...
export default ModalDisabledProduct